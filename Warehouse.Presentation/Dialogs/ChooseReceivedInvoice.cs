...
{
    public class ChooseReceivedInvoice : ChooseInvoice
    {
        public ChooseReceivedInvoice (DocumentChoiceType choiceType)
            : base (choiceType)
        {
        }
        ...
        protected override void InitializeFormStrings ()
        {
            ...
            switch (choiceType) {
                case DocumentChoiceType.Annul:
                    dlgChoose.Title = Translator.GetString ("Void Received Invoice - Select Document");
                    break;
                default:
        ...
        {
            entities = Invoice.GetAllReceived (GetDateFilter ());
        ...
        protected override void AnnulOperation ()
        {
            ...
            if (BusinessDomain.AppConfiguration.AutoCreateInvoiceOnPurchase) {
                if (Message.ShowDialog (Translator.GetString ("Annul Purchases Too?"), null,
                    Translator.GetString ("Do you want to annul the purchases for that invoice too?"), "Icons.Question32.png",
                    MessageButtons.YesNo) == ResponseType.Yes) {
                    operations = Invoice.GetOperationsByReceivedNumber (invoice.Number);
                }
            }
