...
{
    public class ReportFilterChoose : ReportFilterBase<FilterChoose>
    {
        private ComboBox cboValues;

        public ReportFilterChoose (bool alwaysOn, bool enabled, KeyValuePair<int, string> [] values, DataFilterLabel fLabel, params DbField [] fNames)
            : this (new FilterChoose (alwaysOn, enabled, values, fLabel, fNames))
        {
        }

        public ReportFilterChoose (FilterChoose filter)
            : base (filter)
        {
            InitializeWidgets ();
