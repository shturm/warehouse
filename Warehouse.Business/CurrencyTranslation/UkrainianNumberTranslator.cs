...
{
    public class UkrainianNumberTranslator : NumberTranslator
    {
        protected override string GetHundreds (string strNum)
        {
            ...
            switch (hundreds) {
                ...
                case "2":
                    return "двісті" + GetSeparator ();
                case "3":
                case "4":
                    return string.Concat (ConvertDigit (hundreds), "ста", GetSeparator ());
                default:
                    string hundredsCount = ConvertDigit (hundreds);
                    if (hundredsCount.EndsWith ("ь"))
                        hundredsCount = hundredsCount.Substring (0, hundredsCount.Length - 1);

                    return string.Concat (hundredsCount, "сот", GetSeparator ());
            }
        }
        ...
        protected override string GetLargeUnit (string hundreds, string hundredsValue, IList<string> place, int count)
        {
            ...
            if (count == 2) {
                ...
                unit = hundredsNumber == 1 ? unit : unit.Substring (0, unit.Length - 1) + "i";
                ...
                if (hundredsNumber % 10 == 2)
                    return string.Concat (hundreds + (hundredsNumber == 12 ? string.Empty : " двi"), unit, GetSeparator ());
            ...
            if (hundredsNumber > 0 && hundredsNumber < 5)
                return string.Concat (hundreds, unit, "и", GetSeparator ());
            return string.Concat (hundreds, unit, "ів", GetSeparator ());
        }
    }
}
