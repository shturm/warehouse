...
{
    public class ReportQueryInvoicedItems : ReportQueryBase
    {
        public override string HelpFile
        {
            get { return "ReportInvoicedGoods.html"; }
        }

        public ReportQueryInvoicedItems ()
        {
            Name = Translator.GetString ("Invoiced Items Report");
            ...
            KeyValuePair<int, string> [] supportedOperations =
                {
                    new KeyValuePair<int, string> (-1, Translator.GetString ("All")),
                    new KeyValuePair<int, string> ((int) OperationType.Sale, Translator.GetOperationTypeName (OperationType.Sale)),
                    new KeyValuePair<int, string> ((int) OperationType.Purchase, Translator.GetOperationTypeName (OperationType.Purchase))
                };

            AppendFilter (new FilterChoose (false, true, supportedOperations, DataFilterLabel.OperationType, DataField.OperationType));
        ...
        {
            return BusinessDomain.DataAccessProvider.ReportInvoicedItems (dataQuery);
        }
